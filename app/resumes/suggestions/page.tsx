// app/dashboard/resumes/suggestions/page.tsx
"use client";
...
export default function JobSuggestionsPage() {
  ...
  useEffect(() => {
    ...
    if (allResumes.length > 0 && targetCvId) {
      ...
      if (foundResume) {
        ...
        if (!selectedResumeForSuggestion || selectedResumeForSuggestion.id !== foundResume.id) {
        ...
        if (!selectedResumeForSuggestion || selectedResumeForSuggestion.id !== allResumes[0].id) {
 ...
 useEffect(() => {
    if (selectedResumeForSuggestion) {
      ...
      const suggestedJobIds: string[] = (suggestedJobsData as Record<string, string[]>)[selectedResumeForSuggestion.id] || [];
      
      // Lọc ra các jobs tương ứng từ `allJobs`
      const jobs = suggestedJobIds.map(id => allJobs.find(job => job.id === id)).filter(job => job !== undefined) as Job[];
  ...
  const handleSelectResume = useCallback((cv: Resume) => {
    ...
    router.push(`/dashboard/resumes/suggestions?cvId=${cv.id}`, { scroll: false });
  }, [dispatch, router]);
  ...
  const handleToggleJobArchived = (jobId: number) => {
   ...
   const handleHideJob = (jobId: number) => {
    ...
    setSuggestedJobs(prevJobs => prevJobs.filter(job => job.id !== jobId.toString()));
  };
   ...
   if (allResumes.length === 0 && !resumesLoading) {
     return (
      <div className="flex flex-col items-center justify-center h-screen p-4">
        ...
        <p className="text-gray-500 mb-6 text-center">{t("resume.uploadToGetSuggestions")}</p>
          ...
          {selectedResumeForSuggestion ? (
            currentJobs.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-gray-500">
                ...
                <p className="text-sm text-gray-400">{t("job.tryAnotherCvOrSearch")}</p>
